...
def count_capitals(value):
    return size(split(value, r"[A-Z]")) - 1

def has_microsoft_extension(value):
    pattern = r"(\.doc\.)|(\.docx\.)|(\.odt\.)|(\.pages\.)|(\.tf\.)|(\.txt\.)|(\.wpd\.)|(\.wps\.)|(\.csv\.)|(\.numbes\.)|(\.ods\.)|(\.xls\.)|(\.xlsx\.)|(\.asp\.)|(\.aspx\.)|(\.css\.)|(\.htm\.)|(\.html\.)|(\.jsp\.)|(\.php\.)|(\.xml\.)|(\.afdesign\.)|(\.ai\.)|(\.cad\.)|(\.cd\.)|(\.dw\.)|(\.dwg\.)|(\.eps\.)|(\.odg\.)|(\.svg\.)|(\.vsdx\.)|(\.afpub\.)|(\.indd\.)|(\.pdf\.)|(\.pdfxml\.)|(\.pmd\.)|(\.pub\.)|(\.qxp\.)|(\.c\.)|(\.cpp\.)|(\.cs\.)|(\.java\.)|(\.js\.)|(\.json\.)|(\.py\.)|(\.sql\.)|(\.swift\.)|(\.vb\.)|(\.7z\.)|(\.a\.)|(\.ta\.)|(\.ta.gz\.)|(\.zip\.)|(\.bak\.)|(\.cfg\.)|(\.conf\.)|(\.ini\.)|(\.msi\.)|(\.sys\.)|(\.tmp\.)|(\.app\.)|(\.bat\.)|(\.bin\.)|(\.cmd\.)|(\.com\.)|(\.exe\.)|(\.vbs\.)|(\.x86\.)"
    return when(col(value).rlike(pattern),1).otherwise(0)
...
def is_human_readable(entropy_value, threshold=5.0):
    return when(col(entropy_value)<threshold,1).otherwise(0)
...
def process_dnsmasq_for_pred(df_pyspark):
    ...
    df_pyspark = df_pyspark.withColumn("value1_dot_count", count_dots("value1"))
    df_pyspark = df_pyspark.withColumn("value1_hyphen_count", count_hyphens("value1"))
    df_pyspark = df_pyspark.withColumn("value1_slash_count", count_slash("value1"))
    df_pyspark = df_pyspark.withColumn("value1_asterisk_count", count_asterisk("value1"))
    df_pyspark = df_pyspark.withColumn("value1_capital_count", count_capitals(col("value1")))
    df_pyspark = df_pyspark.withColumn("value1_has_file_extensions", has_microsoft_extension("value1"))
    ...
    df_pyspark = df_pyspark.withColumn("value1_human_readable", is_human_readable("entropy"))

    ###########

    df_pyspark = df_pyspark.withColumn("value2_dot_count", count_dots("value2"))
    df_pyspark = df_pyspark.withColumn("value2_hyphen_count", count_hyphens("value2"))
